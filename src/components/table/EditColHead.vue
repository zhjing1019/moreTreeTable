...
export default {
    ...
    methods: {
            ...
            this.edit.resetNewColHead.forEach((x) => {
                ...
                x.forEach((y) => {
                    if(y && y !== null) {
                        ...
                        cell.push(<th colspan={y.rowSpan} rowspan={y.colSpan} style={{ width: this.edit.headColWidth + "px", height: height + "px", textAlign: this.edit.textAlign}}>{thPopover}</th>)
                    }
                })
                tr.push(<tr>{cell}</tr>);
            });
      ...
      popoverCell(h, data, height) {
        let cell = (<standard-cascader data={data} height={height} list={this.edit.initList3}
      ...
      creatFirstHeadTr(h, y, i) {
            if(i == 0) {
                ...
                let thPopover = this.popoverCell(h, y, height);
                return (<th colspan={colspan - 1} rowspan={rowspan} style={{ width: width + "px", height: height + "px", textAlign: this.edit.textAlign,  borderRight: this.edit.moreHead ? "" : "1px solid #d9d9d9", paddingLeft: 0 }}>{thPopover}</th>)
            }
        },
    }
}
